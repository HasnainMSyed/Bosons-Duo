...
with gr.Blocks(theme=gr.themes.Soft(primary_hue="blue")) as app:
    with gr.Tabs(selected=0) as tabs:
        with gr.TabItem("Topic Setup", id=0) as step1_tab:
        ...
        with gr.TabItem("Agent Setup", id=1) as step2_tab:
        ...
        with gr.TabItem("Debate", id=2) as step3_tab:
            gr.HTML(process_debate_js)
    ...
    def process_step1(topic: str, context_file: Optional[Any]) -> str:
        ...
        if context_file:
            ...
            message = f"Received Topic and File: {os.path.basename(context_file)}. Proceeding..."
        else:
            print("No context file uploaded.")
            message = f"Received Topic: '{topic}'. Proceeding..."

        return message, gr.update(selected=1)

    def process_debate() -> None:
    ...
    button_to_configure_agents.click(
        ...
        outputs=[status_output, tabs],  # For now, just show status
        # Later: This button will hide Step 1 UI and show Step 2 UI
    )

    button_to_start_debate.click(
        fn=process_debate,
        inputs=[],
        outputs=[tabs]
    )
